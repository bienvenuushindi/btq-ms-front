...
export default function Form({fields, handleSubmit, childGroup}: {
    fields: any,
    handleSubmit?: any,
    childGroup?: any
}) {
            ...
            {Array.isArray(fields) ? renderFields(fields) :
                <div className="flex ">
                    { Object.keys(fields).map((key, index) =>{
                        return(
                            <div key={key} className={clsx('mx-auto')}>
...
const renderFields= (fields: any[]) => (fields.map((field, index) => {
        return (
            ...
            {Array.isArray(field) ?
                <div className="flex gap-2">
                    {field.map((child, index) => {
                        return (
                            <div key={`form-field-${field.length}-${index}`}
                                 className={`flex flex-col w-1/${field.length}`}>
                                {renderGroup(child)}
                            </div>
                        );
                    })}
