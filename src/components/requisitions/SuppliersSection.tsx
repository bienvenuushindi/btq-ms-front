...
export default function SuppliersSection({action, productId, supplierId, currentSupplier}){
  return (
    <div className="w-72 px-2 rounded">
      <SelectSupplier action={action} productId={productId} supplierId={supplierId}/>
      <Divider/>
      <PreviousSuppliers action={action} productId={productId} supplierId={supplierId} currentSupplier={currentSupplier}/>
    </div>
  )
}
